...
var defaultPromptSections = []promptSection{
	...
	},
	{
		...
		Content: "- Use the minimum set of tools needed to make progress or verify a result safely.\n" +
			...
			"- Set each Todo `executor` explicitly: use `agent` for quick local actions, use `subagent` for parallelizable or long-running tasks.\n" +
			"- Decide execution path before acting: maintain Todos first, then run direct tools or let subagent dispatch handle marked tasks.\n" +
			"- Use `subagent` only when decomposition improves throughput or context isolation; avoid delegating tiny one-shot edits.\n" +
