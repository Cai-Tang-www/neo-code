...
func TestConfigValidateRejectsInvalidInputs(t *testing.T) {
	testCases := []struct {
		...
		{name: "missing listen", cfg: Config{AppID: "app", AppSecret: "secret"}, want: "listen address is required"},
		{name: "missing event path", cfg: Config{ListenAddress: "127.0.0.1:1", AppID: "app", AppSecret: "secret"}, want: "event path is required"},
		{name: "missing card path", cfg: Config{ListenAddress: "127.0.0.1:1", EventPath: "/events", AppID: "app", AppSecret: "secret"}, want: "card path is required"},
